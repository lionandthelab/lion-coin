...
// 위에 둔다.
// 중단 공지인지 판정한다. 재개 규칙과 중단 규칙이 같은 표현을 공유하므로
// 한 곳에 두고 양쪽이 같은 정의를 쓰게 한다.
// c는 공백이 제거된 형태라 '.'로 충분하다.
const SUSPENSION = /입출금.{0,8}(중단|중지)/;

const RULES = [
  {
    kind: '거래유의해제',
    direction: 'bullish',
    // **B급인 이유는 안도 랠리가 작아서가 아니라 재본 적이 없어서다.**
    // 이 재료의 방향은 확실하지만 폭은 이 저장소가 한 번도 측정하지 않았고,
    // A급으로 두면 harness/event-trading.json의 minGrade("A")와 정확히 일치해
    // 자동 진입이 열린다. 방향만 아는 재료는 알림까지다.
    grade: 'B',
    // '유의종목지정'을 포함하므로 반드시 지정 규칙보다 위에 있어야 한다.
    match: (c) => /유의종목지정.{0,4}해제/.test(c),
    note: '유의 종목 지정이 풀렸다. 안도 랠리가 나오지만 폭을 측정한 적이 없어 알림까지만 한다',
  },
  {
    kind: '거래유의',
    direction: 'bearish',
  ...
  },
  {
    ...
    //
    // **중단 공지를 재개로 삼키지 않도록 가른다.** 거래소는 "일시 중단 안내
    // (재개 시 별도 공지)", "중단 및 재개 일정 안내"처럼 중단 공지에 재개를
    // 예고로 얹는다. 그 공지가 알리는 것은 중단이다. 가름의 기준은 해제가
    // 중단에 **직접 붙었는가**로, '중단 해제'만 재개로 본다.
    match: (c) => /입출금.{0,12}(해제|재개)/.test(c)
      && (!SUSPENSION.test(c) || /(중단|중지)(해제|해지)/.test(c)),
