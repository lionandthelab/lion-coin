...
//
// ⚠ 안전 규약
//   - **최초** 실거래 전환에는 .env의 BITHUMB_LIVE=1 승인과 화면에서의 명시적
//     확인이 **둘 다** 필요하다. 이 프로세스는 스스로 그 전환을 만들지 않는다.
//   - **재시작은 그 선택을 꺼버리지 않는다.** 실거래 중이었다면 다시 실거래로
//     들어간다 — 재시작이 사람이 내린 결정을 되돌리는 것도 사고이기 때문이다.
//     다만 .env의 승인이나 API 키가 사라졌으면 감시 모드로 떨어뜨린다.
//     승인을 내린 것은 명시적인 의사표시이고, 저장된 값이 그걸 이기면 안 된다.
//   - **모드보다 상태를 먼저 되살린다.** 순서가 뒤집히면 이전 포지션을 모르는 채
//     새 재료로 또 진입한다 — 크래시 루프에서 감시받지 않는 포지션이 쌓인다.
...
server.listen(PORT, () => {
  ...
  console.log(mode === 'live'
    ? '  🔴 실거래 중입니다. 재시작 전 상태를 이어받았습니다 — 끄려면 화면에서 정지하거나 .env의 BITHUMB_LIVE를 내리세요.'
    : '  ⚠ 실거래 최초 전환은 화면에서만 됩니다. 한 번 켜두면 재시작 뒤에도 이어집니다.');
});
