...
- **왜:** 실제 sats를 수령할 계좌. 이것 없이는 목표 판정 자체가 불가능합니다.
  프로젝트 시작(2026-07-14) 이래 39일 넘게 이 항목 하나가 목표 달성의 유일한 병목입니다.
- **왜 Coinos인가:**
...
- (2026-08-22 갱신) 지금 `.env`에 남아 있는 옛 `BLINK_API_KEY`는 사실 Blink 키가 아니라 다른 서비스의 키(`ak_v2_` 접두사)라 형식부터 틀렸습니다. 이전에는 이 때문에 `check-goal`이 매 회차 401로 죽었지만 오늘 회차에서 고쳐서, 이제는 죽지 않고 `configured:false`와 함께 어떤 키가 왜 틀렸는지(`providerError`)를 보여줍니다. 어차피 쓸 수 없는 키이니 COINOS_TOKEN을 넣을 때 함께 지우는 걸 권장합니다.
